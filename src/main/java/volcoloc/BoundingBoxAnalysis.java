 ...
 */
final class BoundingBoxAnalysis {
    ...
    BoundingBoxAnalysis(VolColocParameters parameters,
    ...
    List<VolColocResult.BoundingBoxDirectionResult> run() {
        for (ImagePlus image : images) geometries.add(extract(image));
        List<VolColocResult.BoundingBoxDirectionResult> results =
                new ArrayList<VolColocResult.BoundingBoxDirectionResult>();
    ...
    private VolColocResult.BoundingBoxDirectionResult buildDirection(
            int sourceIndex, int targetIndex, ExecutorService executor) {
        List<Geometry> sources = geometries.get(sourceIndex);
        List<Geometry> partners = geometries.get(targetIndex);
        List<VolColocResult.BoundingBoxObjectResult> rows =
                new ArrayList<VolColocResult.BoundingBoxObjectResult>(sources.size());
        double threshold = thresholds.get(sourceIndex).doubleValue();
        ...
        if (executor == null || sources.size() < 2 || !safeParallelRead) {
            for (Geometry source : sources) {
                rows.add(buildObjectRow(
                        source, partners, targetIndex, threshold));
            }
        } else {
            List<Future<VolColocResult.BoundingBoxObjectResult>> futures =
                    new ArrayList<Future<VolColocResult.BoundingBoxObjectResult>>(sources.size());
            for (final Geometry source : sources) {
                futures.add(executor.submit(
                        new Callable<VolColocResult.BoundingBoxObjectResult>() {
                            @Override
                            public VolColocResult.BoundingBoxObjectResult call() {
        ...
        return new VolColocResult.BoundingBoxDirectionResult(
                sourceIndex,
                targetIndex,
    ...
    private VolColocResult.BoundingBoxObjectResult buildObjectRow(
            ...
            return new VolColocResult.BoundingBoxObjectResult(
                    source.label,
                    source.boxVolume(),
                    boxOverlapPercent,
                    boxOverlapPartner,
